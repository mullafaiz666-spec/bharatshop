...
const collectionCards = [
  { label: "Outerwear", number: "01", copy: "Layers with a point of view.", image: staticProducts[0].images[1], color: "sand" },
  { label: "Daily uniform", number: "02", copy: "The pieces you'll live in.", image: staticProducts[1].images[1], color: "sage" },
  { label: "After dark", number: "03", copy: "Made for the long way home.", image: staticProducts[5].images[0], color: "wine" },
];

export function Storefront({ initialProducts = staticProducts }: { initialProducts?: Product[] }) {
  ...
  }, [activeCategory, initialProducts, searchTerm, sort]);
  ...
  return (
    <main>
      ...
      <section className="shop-section page-section" id="shop">
        ...
        <div className="shop-toolbar"><div className="category-tabs" role="tablist" aria-label="Filter products">{(["All", "Tops", "Hoodies", "Outerwear", "Bottoms", "Accessories"] as ProductCategory[]).map((category) => { const count = category === "All" ? initialProducts.length : initialProducts.filter((product) => product.category === category).length; if (category !== "All" && count === 0) return null; return <button key={category} className={activeCategory === category ? "active" : ""} onClick={() => setActiveCategory(category)}>{category}{category !== "All" && <sup>{count}</sup>}</button>; })}</div><label className="sort-select"><span>Sort by</span><select value={sort} onChange={(event) => setSort(event.target.value)}><option value="featured">Featured</option><option value="price-low">Price: low to high</option><option value="price-high">Price: high to low</option><option value="rating">Top rated</option></select><ChevronDown size={15} /></label></div>
