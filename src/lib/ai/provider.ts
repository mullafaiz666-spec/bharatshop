...
const requestTimeoutMs = () => Math.max(15000, Number(process.env.AI_REQUEST_TIMEOUT_MS || 180000));
const readinessTimeoutMs = () => Math.max(10000, Number(process.env.AI_READINESS_TIMEOUT_MS || 60000));

function headers() { const key = apiKey(); return { "Content-Type": "application/json", ...(key ? { Authorization: `Bearer ${key}` } : {}) }; }

async function request(path: string, body: unknown, timeoutMs = requestTimeoutMs()) {
  const base = baseUrl();
...
export async function runAI(messages: AIMessage[], options: ProviderOptions = {}) {
  ...
  const data = await request("/chat/completions", { model: options.model || models.text, messages, temperature: options.temperature ?? 0.2, max_tokens: options.maxTokens ?? 2048, ...(options.tools ? { tools: options.tools, tool_choice: options.toolChoice ?? "auto" } : {}) }, options.timeoutMs ?? requestTimeoutMs());
  return data;
}
...
export async function verifyImagesWithAI(images: Array<{ url: string; data: string; mediaType: string }>, product: { title: string; brand: string; category: string }) {
  ...
  const data = await runAI([{ role: "user", content }], { model: aiModels().vision, temperature: 0, maxTokens: 3000, timeoutMs: requestTimeoutMs() });
...
export async function checkAI(deep = false) {
  ...
  try {
    const res = await fetch(`${base}/models`, { headers: headers(), cache: "no-store", signal: AbortSignal.timeout(readinessTimeoutMs()) });
    const body = await res.text();
    if (!res.ok) return { configured: true, ready: false, status: res.status, reason: "provider_rejected", detail: body.slice(0, 300), provider: aiProviderName(), models: aiModels() };
    if (!deep) return { configured: true, ready: true, status: res.status, reason: "reachable", provider: aiProviderName(), models: aiModels() };
    const probe = await runText([{ role: "user", content: "Reply with exactly OK." }], { maxTokens: 16, timeoutMs: requestTimeoutMs() });
