...
export async function listPendingApprovals(){await ensureApprovalTable();const result=await pool.query(`SELECT id,title,action_type,payload,reason,risk_level,status,created_at FROM ceo_approvals WHERE status='PENDING' ORDER BY created_at DESC LIMIT 20`);return result.rows;}
