...
export async function createApproval(input:{title:string;actionType:string;payload?:unknown;reason:string;riskLevel?:string}){await ensureApprovalTable();const result=await pool.query(`INSERT INTO ceo_approvals (title,action_type,payload,reason,risk_level) VALUES ($1,$2,$3,$4,$5) RETURNING *`,[input.title,input.actionType,JSON.stringify(input.payload??{}),input.reason,input.riskLevel??"MEDIUM"]);return result.rows[0];}
export async function listPendingApprovals(){await ensureApprovalTable();const result=await pool.query(`SELECT id,title,action_type,payload,reason,risk_level,status,created_at FROM ceo_approvals WHERE status='PENDING' AND title NOT ILIKE 'Production acceptance%' ORDER BY created_at DESC LIMIT 20`);return result.rows;}
export async function rejectProduct(productId?:number,productName?:string,reason="Product failed verification"){
 const selector=productId?`p.id = $1`:`p.title ILIKE $1`,value=productId?productId:`%${String(productName||"").trim()}%`;const result=await pool.query(`SELECT p.id,p.title,p.sku,p.status FROM products p WHERE ${selector} ORDER BY p.id LIMIT 1`,[value]);
 if(!result.rows.length)return{success:false,error:"Product not found",productId,productName};const product=result.rows[0];await pool.query(`UPDATE products SET status='Rejected',updated_at=NOW() WHERE id=$1`,[product.id]);
 try{await pool.query(`INSERT INTO ai_activity_logs (user_id,agent_name,action_type,message,metadata_json,status) VALUES (1,'AI CEO','PRODUCT_REJECTED',$1,$2,'SUCCESS')`,[`Rejected ${product.title} (${product.sku}). Reason: ${reason}`,JSON.stringify({productId:product.id,sku:product.sku,reason})]);}catch{}
...
export async function inspectLiveBusinessData(){
 const[products,orders,storefrontOrders,activity,refreshes,approvals]=await Promise.all([
  pool.query(`SELECT COUNT(*)::int AS total,COUNT(*) FILTER (WHERE UPPER(status)='PUBLISHED')::int AS published,COUNT(*) FILTER (WHERE UPPER(status) IN ('CEO_PENDING','CEO_APPROVED'))::int AS ceo_pending,COUNT(*) FILTER (WHERE UPPER(status) IN ('STAGED','AI_DRAFT'))::int AS staged,COUNT(*) FILTER (WHERE UPPER(status) LIKE '%REJECT%' OR UPPER(status) LIKE '%BLOCK%')::int AS rejected,COUNT(*) FILTER (WHERE image_url IS NULL OR image_url='')::int AS missing_images,COUNT(*) FILTER (WHERE supplier_name='Qikink' AND brand='BharatShop Studio')::int AS qikink_fashion FROM products`),
  pool.query(`SELECT COUNT(*)::int AS total,COALESCE(SUM(customer_paid_inr),0)::numeric AS revenue,COALESCE(SUM(net_profit_inr),0)::numeric AS profit,COUNT(*) FILTER (WHERE fulfillment_status IN ('RECHECK_REQUIRED','Pending','Received','QIKINK_PRODUCTION_PENDING','QIKINK_SUBMISSION_APPROVAL_REQUIRED'))::int AS pending FROM orders`),
  pool.query(`SELECT COUNT(*)::int AS total,COALESCE(SUM(total_amount_inr),0)::numeric AS revenue,COUNT(*) FILTER (WHERE fulfillment_status IN ('RECHECK_REQUIRED','Pending','Received','QIKINK_PRODUCTION_PENDING','QIKINK_SUBMISSION_APPROVAL_REQUIRED'))::int AS pending FROM storefront_orders`),
