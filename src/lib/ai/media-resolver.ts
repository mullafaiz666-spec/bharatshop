...
import { aiModels, verifyImagesWithAI, visionProviderName } from "@/lib/ai/provider";
...
async function resolveOne(productId?: number, productName?: string) {
  ...
  const provider = visionProviderName();
  const approved = existing.filter(x => String(x.verificationStatus) === "AI_VISION_VERIFIED" && !BAD.test(x.imageUrl) && /^https:\/\//i.test(x.imageUrl) && Number(x.verificationConfidence) >= MIN_CONFIDENCE && String(x.verificationProvider) === provider && String(x.verificationModel) === model && !!x.verifiedAt).sort((a,b) => a.sortOrder - b.sortOrder);
  if (approved.length >= MIN_IMAGES) { const publishable = publicationData(product); if (publishable && product.status !== "Published") await db.update(products).set({ status: "Published", imageUrl: approved[0].imageUrl, updatedAt: new Date() }).where(eq(products.id, product.id)); return { status: "COMPLETE_MEDIA_RESOLVED", provider: "postgres-cache", verificationProvider: provider, productId: product.id, product: product.title, imageCount: Math.min(approved.length, MAX_IMAGES), images: approved.slice(0, MAX_IMAGES).map(x => ({ url: x.imageUrl, confidence: Number(x.verificationConfidence), reason: String((x.verificationMetadata as any)?.reason || "Previously Anthropic-verified") })), cached: true, publicationGate: publishable ? "PASS" : "BLOCK" }; }
  ...
  catch (e) { const result = { status: "VERIFICATION_FAILED", productId: product.id, product: product.title, error: e instanceof Error ? e.message : "Anthropic vision verification failed", provider, model, publicationGate: "BLOCK", message: "Anthropic vision verification could not run. Product remains staged." }; cacheFailure(product.id, result); return result; }
  ...
  if (accepted.length < MIN_IMAGES) { const result = { status: "NEEDS_IMAGES", productId: product.id, product: product.title, imageCount: accepted.length, verdicts, searched: queries, publicationGate: "BLOCK", provider, model, message: `Only ${accepted.length} image(s) passed Anthropic vision verification (need ${MIN_IMAGES}). Product remains staged.` }; cacheFailure(product.id, result); return result; }
  ...
  return { status: "COMPLETE_MEDIA_RESOLVED", provider: "searxng+anthropic-vision", verificationProvider: provider, model, productId: product.id, product: product.title, imageCount: accepted.length, images: accepted.map(v => ({ url: v.item.candidate.url, confidence: v.confidence, reason: v.reason })), cached: false, publicationGate: publishable ? "PASS" : "BLOCK", message: publishable ? `${accepted.length} images passed Anthropic vision verification and the product was published.` : `${accepted.length} images passed Anthropic vision verification, but pricing/stock validation failed; product remains staged.` };
}
