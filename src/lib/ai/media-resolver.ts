...
// Pipeline: PostgreSQL cache -> SearXNG candidate search -> safe URL/content
// download -> Claude vision verification -> only verified images are saved.
// Never falls back to a random/generic stock photo.
// ─────────────────────────────────────────────────────────────────────────────
...
async function fetchImageAsBase64(url: string): Promise<{ data: string; mediaType: string } | null> {
  try {
    const res = await fetch(url, { signal: AbortSignal.timeout(10000), redirect: "follow", cache: "no-store" });
    if (!res.ok) return null;
    const contentType = (res.headers.get("content-type") || "").split(";")[0].toLowerCase();
    if (!contentType.startsWith("image/")) return null;
    const buf = await res.arrayBuffer();
    if (!buf.byteLength || buf.byteLength > 5_000_000) return null;
    return { data: Buffer.from(buf).toString("base64"), mediaType: contentType };
  } catch {
    return null;
  }
}
...
async function verifyImagesWithClaude(candidates: ImageCandidate[], product: Product): Promise<VisionVerdict[]> {
  ...
  const content: any[] = [{
    type: "text",
    text: `Product: "${product.title}" | Brand: ${product.brand} | Category: ${product.category}\n\n` +
      `Evaluate ${usable.length} candidate images labeled Image 1 through Image ${usable.length}. ` +
      `For EACH image, decide whether it genuinely depicts the named product: correct product type, brand marks when visible, form factor, and stated color/variant. ` +
      `Reject generic stock photos, lifestyle images that do not clearly show the product, wrong variants, and unrelated collages.\n\n` +
      `Respond ONLY with a JSON array, one object per image, in order: [{"index":1,"matches":true,"confidence":0.9,"reason":"..."}]`,
  }];
  ...
  return parsed.map((v: any) => {
    ...
    return { url: c?.url || "", matches: !!v.matches, confidence: Number(v.confidence) || 0, reason: String(v.reason || "") };
  }).filter((v) => v.url);
}
...
async function resolveOne(productId?: number, productName?: string) {
  ...
  const approved = existing.filter((x) => String(x.verificationStatus) === "AI_VISION_VERIFIED" && !BAD.test(x.imageUrl)).sort((a, b) => a.sortOrder - b.sortOrder);
  if (approved.length >= MIN_IMAGES) {
    return { status: "COMPLETE_MEDIA_RESOLVED", provider: "postgres-cache", productId: product.id, product: product.title, imageCount: Math.min(approved.length, MAX_IMAGES), images: approved.slice(0, MAX_IMAGES).map((x) => ({ url: x.imageUrl, confidence: 1, reason: "Previously AI-vision-verified" })), cached: true };
  }
  ...
  // Text matching is ranking only, never a hard gate. Search engines commonly
  // return CDN/proxy URLs whose URL contains none of the product keywords.
  ...
  if (!preFiltered.length) return { status: "NEEDS_IMAGES", productId: product.id, product: product.title, imageCount: 0, searched: queries, message: "No image candidates returned by search." };

  let verdicts: VisionVerdict[];
  try {
    verdicts = await verifyImagesWithClaude(preFiltered, product);
  } catch (e) {
    return { status: "VERIFICATION_FAILED", productId: product.id, product: product.title, error: e instanceof Error ? e.message : "Claude vision verification failed", message: "Vision verification could not run. Listing left unchanged." };
  }

  const accepted = verdicts.filter((v) => v.matches && v.confidence >= MIN_CONFIDENCE).sort((a, b) => b.confidence - a.confidence).slice(0, MAX_IMAGES);
  if (accepted.length < MIN_IMAGES) return { status: "NEEDS_IMAGES", productId: product.id, product: product.title, imageCount: accepted.length, verdicts, searched: queries, message: `Only ${accepted.length} image(s) passed vision verification (need ${MIN_IMAGES}). Listing left unchanged.` };
...
export async function resolveVerifiedMediaForProducts(productList: Array<{ id: number }>, maxBatch = 25) {
  ...
  for (const p of batch) {
    ...
    catch (e) { out.push({ status: "ERROR", productId: p.id, error: e instanceof Error ? e.message : "resolve failed" }); }
  }
  return out;
}
