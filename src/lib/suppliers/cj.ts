...
export type CjProduct = {
  id: string; nameEn: string; sku: string; bigImage: string; sellPrice: string; nowPrice: string;
  threeCategoryName?: string; supplierName?: string; warehouseInventoryNum?: number;
  totalVerifiedInventory?: number; verifiedWarehouse?: number; deliveryCycle?: string;
  description?: string; directMinOrderNum?: string;
};
...
async function getAccessToken(): Promise<string> {
  ...
  const res = await fetch(`${BASE}/authentication/getAccessToken`, {
    ...
    body: JSON.stringify({ apiKey: apiKey() }), cache: "no-store",
  });
...
async function cjFetch(path: string, init: RequestInit = {}) {
  ...
  if (!res.ok || data?.result === false || data?.success === false) {
    if (res.status === 401) { accessToken = null; accessTokenExpiry = 0; }
...
export async function ensureSupplierLinkTable() {
  await db.execute(sql`
    CREATE TABLE IF NOT EXISTS supplier_product_links (
      id SERIAL PRIMARY KEY, product_id INTEGER NOT NULL, provider TEXT NOT NULL,
      supplier_product_id TEXT NOT NULL, supplier_variant_id TEXT, supplier_sku TEXT,
      supplier_cost_usd NUMERIC(12,2), verified_inventory INTEGER NOT NULL DEFAULT 0,
      source_url TEXT, last_synced_at TIMESTAMP NOT NULL DEFAULT NOW(),
...
export async function searchCjProducts(options: { keyword?: string; limit?: number; countryCode?: string }) {
  ...
  const qs = new URLSearchParams({ page: "1", size: String(size), verifiedWarehouse: "1", sort: "desc", orderBy: "4", features: "enable_description,enable_category" });
...
export async function importCjProducts(args: { keyword?: string; limit?: number; userId: number }) {
  ...
  for (const p of products) {
    ...
    const gst = 18;
    const inventory = Number(p.totalVerifiedInventory || p.warehouseInventoryNum || 0);
    const profit = priceInr - costInr - shippingInr - costInr * gst / 100;
    if (profit <= 0 || inventory <= 0) continue;
    const sku = `CJ-${p.sku || p.id}`;
    const [row] = await db.execute(sql`
      INSERT INTO products (user_id, sku, title, category, image_url, brand, supplier_name, supplier_city,
        supplier_cost_inr, shipping_cost_inr, gst_pct, selling_price_inr, mrp_inr, custom_margin_pct,
        net_profit_inr, ai_score, viral_velocity_score, stock_count, moq, auto_reprice_enabled, status,
        ai_marketing_copy, ai_target_audience, hsn_code, sales_count_24h, returns_count, created_at, updated_at)
      VALUES (${args.userId}, ${sku}, ${p.nameEn}, ${p.threeCategoryName || "General"}, ${p.bigImage || "https://placehold.co/600x600?text=Product"},
        ...
        ${minMargin.toFixed(2)}, ${profit.toFixed(2)}, 90, 85, ${inventory}, ${Math.max(Number(p.directMinOrderNum || 1), 1)}, true, "Published",
    ...
    await db.execute(sql`INSERT INTO supplier_product_links (product_id, provider, supplier_product_id, supplier_sku, supplier_cost_usd, verified_inventory, source_url)
      VALUES (${productId}, 'cj', ${p.id}, ${p.sku || null}, ${costUsd.toFixed(2)}, ${inventory}, ${`https://www.cjdropshipping.com/product/${p.id}.html`})
