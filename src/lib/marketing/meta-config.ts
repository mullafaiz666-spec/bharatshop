...
export function metaConfiguration() {
  ...
  const server = value("META_PIXEL_ID") || browser;
  const version = value("META_GRAPH_API_VERSION") || "v26.0";
  const issues: string[] = [];
  for (const key of ["NEXT_PUBLIC_META_PIXEL_ID", "META_PIXEL_ID", "META_PAGE_ID", "META_INSTAGRAM_ACCOUNT_ID", "META_CATALOG_ID"]) {
  ...
  if (browser && server && browser !== server) issues.push("META_PIXEL_ID and NEXT_PUBLIC_META_PIXEL_ID must match for event deduplication");
  const token = value("META_CONVERSIONS_API_TOKEN") || value("META_ACCESS_TOKEN");
  return {
    status: issues.length ? "INVALID" : browser || server || value("META_ACCESS_TOKEN") ? "NOT_VERIFIED" : "NOT_CONFIGURED",
    issues,
    browserPixelConfigured: /^\d+$/.test(browser),
    conversionsApiConfigured: /^\d+$/.test(server) && Boolean(token) && !issues.length,
    ...
    requiredForServerTracking: ["META_PIXEL_ID or NEXT_PUBLIC_META_PIXEL_ID", "META_CONVERSIONS_API_TOKEN or META_ACCESS_TOKEN"],
