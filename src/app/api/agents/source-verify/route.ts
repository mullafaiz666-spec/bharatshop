...
export async function POST(request:Request){
 try{
  const body=await request.json();let candidates:Candidate[]=Array.isArray(body.candidates)?body.candidates:[];const query=String(body.productName||body.title||"").trim();const min=Number(body.minMarginPct??35);const fallbackSelling=Number(body.sellingPriceInr||0);const productId=Number(body.productId||0);let persistedProduct:any=null;
  if(productId){
  ...
  for(const candidate of candidates){
   ...
   const evidenceEligible=Boolean(evidence.reachable&&evidence.titleMatch&&evidence.priceVerified&&evidence.stockVerified&&evidence.stockAvailable===true&&evidence.shippingVerified&&selling>0&&Number.isFinite(profit)&&profit>0&&Number.isFinite(margin)&&margin>=min);
   const eligible=Boolean(evidenceEligible&&policy.fulfillmentAllowed);
   evaluated.push({...candidate,sourcePolicy:policy,supplierCostInr:verifiedPrice,shippingCostInr:Number.isFinite(shipping)?shipping:null,sellingPriceInr:selling,economics:{landedCostInr:Number.isFinite(landed)?+landed.toFixed(2):null,netProfitInr:Number.isFinite(profit)?+profit.toFixed(2):null,marginPct:Number.isFinite(margin)?+margin.toFixed(2):null,selectionScore:eligible?Math.round(Math.min(100,Number(margin))):0},sourceEvidence:evidence,benchmarkEligible:Boolean(policy.benchmarkAllowed&&verifiedPrice>0),eligible});
  }

  const ai=await openAIJson(agentPrompt("source-verification"),{query:effectiveQuery,min,evaluated,marketplacePolicy:marketplacePolicySummary});
  ...
  if(valid&&productId){
   ...
   await db.transaction(async tx=>{
    ...
    const persistedSpecs={...(existingDetails?.specificationsJson as Record<string,unknown>||{}),sourcePolicy:valid.sourcePolicy,sourceVerification:{verifiedAt:now.toISOString(),benchmarkSources:evaluated.filter(x=>x.benchmarkEligible).slice(0,5).map(x=>({sourceName:x.sourceName,sourceUrl:x.sourceUrl,priceInr:x.supplierCostInr,lane:x.sourcePolicy?.lane}))}};
  ...
  await db.insert(aiActivityLogs).values({userId:Number(body.userId??persistedProduct?.userId??1),agentName:"Verify-Select-AI",actionType:"SOURCE_VERIFICATION_COMPLETED",message:`Verified ${evaluated.length} marketplace/direct supplier candidates against live evidence, economics and source policy.`,profitImpactInr:String(valid?.economics.netProfitInr??0),metadataJson:{productId:productId||null,evaluated,selected:valid,ai,persisted:Boolean(valid&&productId)},status:valid?"SUCCESS":"WARNING"});
  return NextResponse.json({pipeline:"Saved discovery + Meesho/Shopsy/Flipkart/Amazon intelligence + direct supplier search → Source Policy → Live Evidence → Local Gemma → Economics → Select",candidates:evaluated,selected:valid,status:valid?"READY_FOR_LISTING":"NO_QUALIFIED_PRODUCT",persisted:Boolean(valid&&productId),ai,marketplacePolicy:marketplacePolicySummary});
 }catch(error){return NextResponse.json({error:error instanceof Error?error.message:"Invalid request"},{status:503})}
}
export async function GET(){const ai=Boolean(process.env.AI_BASE_URL||process.env.LOCAL_AI_BASE_URL);return NextResponse.json({agent:"Verify-Select-AI",status:process.env.SEARXNG_URL&&ai?"ready":"blocked_missing_provider",capabilities:["saved_discovery_source","meesho_shopsy_flipkart_amazon_intelligence","direct_supplier_search","live_source_page_evidence","source_policy","local_gemma_verification","economics_gate","verified_source_persistence"],marketplacePolicy:marketplacePolicySummary});}
