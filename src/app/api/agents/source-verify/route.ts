...
export async function POST(request:Request){
 try{
  ...
  for(const candidate of candidates){
   ...
   const freshnessCurrent=Boolean(evidence.freshness?.price.current&&evidence.freshness?.availability.current&&evidence.freshness?.shipping.current&&evidence.freshness?.productUrl.current);
   const evidenceEligible=Boolean(evidence.reachable&&evidence.titleMatch&&evidence.priceVerified&&evidence.stockVerified&&evidence.stockAvailable===true&&evidence.shippingVerified&&freshnessCurrent&&selling>0&&Number.isFinite(profit)&&profit>=minProfit&&Number.isFinite(margin)&&margin>=min);
   const eligible=Boolean(evidenceEligible&&policy.fulfillmentAllowed);
   evaluated.push({...candidate,sourcePolicy:policy,supplierCostInr:verifiedPrice,shippingCostInr:Number.isFinite(shipping)?shipping:null,sellingPriceInr:selling,economics:{landedCostInr:Number.isFinite(landed)?+landed.toFixed(2):null,netProfitInr:Number.isFinite(profit)?+profit.toFixed(2):null,marginPct:Number.isFinite(margin)?+margin.toFixed(2):null,minMarginPct:min,minProfitInr:minProfit,selectionScore:eligible?Math.round(Math.min(100,Number(margin)+Math.min(20,Number(profit)/Math.max(1,minProfit)))):0},sourceEvidence:evidence,freshnessCurrent,benchmarkEligible:Boolean(policy.benchmarkAllowed&&verifiedPrice>0),eligible});
  }
  ...
  if(valid&&productId){
   ...
   await db.transaction(async tx=>{
    ...
    const persistedSpecs={...(existingDetails?.specificationsJson as Record<string,unknown>||{}),economicsPolicy:{minMarginPct:min,minProfitInr:minProfit},sourcePolicy:valid.sourcePolicy,sourceVerification:{verifiedAt:now.toISOString(),checkedAt:valid.sourceEvidence.checkedAt,finalUrl:valid.sourceEvidence.finalUrl||valid.sourceUrl,freshness:valid.sourceEvidence.freshness,verifiedSignals:{titleMatch:valid.sourceEvidence.titleMatch,priceVerified:valid.sourceEvidence.priceVerified,stockVerified:valid.sourceEvidence.stockVerified,stockAvailable:valid.sourceEvidence.stockAvailable,shippingVerified:valid.sourceEvidence.shippingVerified},benchmarkSources:evaluated.filter(x=>x.benchmarkEligible).slice(0,5).map(x=>({sourceName:x.sourceName,sourceUrl:x.sourceUrl,priceInr:x.supplierCostInr,lane:x.sourcePolicy?.lane}))}};
  ...
  await db.insert(aiActivityLogs).values({userId:Number(body.userId??persistedProduct?.userId??1),agentName:"Verify-Select-AI",actionType:"SOURCE_VERIFICATION_COMPLETED",message:`Verified ${evaluated.length} marketplace/direct supplier candidates against live evidence freshness, category economics and source policy.`,profitImpactInr:String(valid?.economics.netProfitInr??0),metadataJson:{productId:productId||null,category:persistedProduct?.category||body.category||null,minMarginPct:min,minProfitInr:minProfit,evaluated,selected:valid,ai,persisted:Boolean(valid&&productId)},status:valid?"SUCCESS":"WARNING"});
  return NextResponse.json({pipeline:"Saved discovery + marketplace/direct supplier search → Source Policy → Live Evidence → Freshness → Category Economics → Local Gemma → Select",candidates:evaluated,selected:valid,status:valid?"READY_FOR_LISTING":"NO_QUALIFIED_PRODUCT",persisted:Boolean(valid&&productId),economicsPolicy:{minMarginPct:min,minProfitInr:minProfit},ai,marketplacePolicy:marketplacePolicySummary});
 }catch(error){return NextResponse.json({error:error instanceof Error?error.message:"Invalid request"},{status:503})}
}
export async function GET(){const ai=Boolean(process.env.AI_BASE_URL||process.env.LOCAL_AI_BASE_URL);return NextResponse.json({agent:"Verify-Select-AI",status:process.env.SEARXNG_URL&&ai?"ready":"blocked_missing_provider",capabilities:["saved_discovery_source","meesho_shopsy_flipkart_amazon_intelligence","direct_supplier_search","live_source_page_evidence","evidence_freshness","source_policy","category_economics_gate","local_gemma_verification","verified_source_persistence"],marketplacePolicy:marketplacePolicySummary,promptVersion:"agent-suite-v4"});}
