 ...
 try{
  ...
  if(valid&&productId){
   ...
   await db.transaction(async tx=>{
    ...
    const persistedSpecs={...(existingDetails?.specificationsJson as Record<string,unknown>||{}),sourcePolicy:valid.sourcePolicy};
  ...
  await db.insert(aiActivityLogs).values({userId:Number(body.userId??1),agentName:"Verify-Select-AI",actionType:"SOURCE_VERIFICATION_COMPLETED",message:`Verified ${evaluated.length} source candidates against live evidence, economics and source policy.`,profitImpactInr:String(valid?.economics.netProfitInr??0),metadataJson:{productId:productId||null,evaluated,selected:valid,ai,persisted:Boolean(valid&&productId)},status:valid?"SUCCESS":"WARNING"});
  return NextResponse.json({pipeline:"Search → Source Policy → Live Evidence → Local Gemma → Economics → Select",candidates:evaluated,selected:valid,status:valid?"READY_FOR_LISTING":"NO_QUALIFIED_PRODUCT",persisted:Boolean(valid&&productId),ai,marketplacePolicy:marketplacePolicySummary});
 }catch(error){return NextResponse.json({error:error instanceof Error?error.message:"Invalid request"},{status:503})}
}
export async function GET(){const ai=Boolean(process.env.AI_BASE_URL||process.env.LOCAL_AI_BASE_URL);return NextResponse.json({agent:"Verify-Select-AI",status:process.env.SEARXNG_URL&&ai?"ready":"blocked_missing_provider",capabilities:["live_source_page_evidence","source_policy","local_gemma_verification","economics_gate","verified_source_persistence"],marketplacePolicy:marketplacePolicySummary});}
