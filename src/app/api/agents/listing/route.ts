...
export async function POST(req: Request) {
  try {
    const { productId } = await req.json(); if (!productId) return NextResponse.json({ error:"productId required" },{status:400});
    ...
    const listing={title,description,sellingPriceInr:selling,marginPct:+margin.toFixed(2),netProfitInr:+profit.toFixed(2),marketingCopy,targetAudience,sourceEvidence:{imageUrl:verifiedImage.imageUrl,sourceUrl:verifiedImage.sourceUrl,sourceName:verifiedImage.altText,verificationStatus:verifiedImage.verificationStatus},adCreativeData:{hook:String(ai.hook||marketingCopy),audience:targetAudience,imageUrl:verifiedImage.imageUrl,cta:String(ai.cta||"Abhi Kharido")}};
