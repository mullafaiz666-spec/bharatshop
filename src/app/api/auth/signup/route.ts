...
export async function POST(req:Request){
  try{
    ...
    const [user]=await db.insert(users).values({name,email,passwordHash:hash(password),role:"Customer"}).returning({id:users.id,name:users.name,email:users.email,role:users.role});
    return NextResponse.json({ok:true,user});
  }catch(e){
    return NextResponse.json({error:e instanceof Error?e.message:"Unable to create account"},{status:503});
  }
}
