...
function score(item:any,p:any){const ts=tokens(p.title);const hay=`${item.title||""} ${item.source||""} ${item.link||""}`.toLowerCase();const hits=ts.filter(t=>hay.includes(t)).length;const ratio=ts.length?hits/ts.length:0;const brand=String(p.brand||"").trim().toLowerCase();return ratio+(brand&&brand!=="generic"&&hay.includes(brand)?0.25:0);}
...
export async function POST(req:Request){
 try{
  ...
  const candidates=all.filter(p=>!exact.has(p.id)).slice(0,limit);let resolved=0,rejected=0;const results=[];
  for(const p of candidates){const fashion=FASHION.test(`${p.category} ${p.title}`);const q=`${p.title} ${p.brand!=="Generic"?p.brand:""} exact product official supplier image`;let found=[];try{found=await searchImages(q,key);}catch(e){results.push({productId:p.id,status:"SEARCH_ERROR",error:e instanceof Error?e.message:"search failed"});continue;}
   const ranked=found.filter(x=>x.original&&/^https?:\/\//i.test(x.original)&&x.link&&!BAD.test(x.original)).map(x=>({...x,score:score(x,p)})).sort((a,b)=>b.score-a.score);const best=ranked[0];const threshold=fashion?0.70:0.45;
