...
export async function POST(req:Request){
 try{
  ...
  const candidates=all.filter(p=>(counts.get(p.id)||0)<5).slice(0,limit);let resolved=0,rejected=0;const results:Array<Record<string,unknown>>=[];
  for(const p of candidates){
   const fashion=FASHION.test(`${p.category} ${p.title}`);
   const base=`${p.title} ${p.brand!=="Generic"?p.brand:""}`;
   const queries=[`${base} exact product official image front back side packaging`,`${base} exact product box packaging contents`,`${base} exact product colour variants colors`];
   let found:ImageSearchResult[]=[];
   try{for(const q of queries){found.push(...await searchImages(q,key));}}catch(e){results.push({productId:p.id,status:"SEARCH_ERROR",error:e instanceof Error?e.message:"search failed"});continue;}
   ...
   for(const item of ranked){if(selected.length>=5)break;if(Number(item.score)<threshold||seen.has(item.original!))continue;seen.add(item.original!);selected.push(item);}
   ...
   await db.update(products).set({imageUrl:selected[0].original!,updatedAt:new Date()}).where(eq(products.id,p.id));
   resolved++;results.push({productId:p.id,status:"EXACT_GALLERY_RESOLVED",imageCount:selected.length,scores:selected.map(x=>Number(x.score.toFixed(3))),sources:selected.map(x=>x.link)});
  }
  return NextResponse.json({status:"COMPLETED",processed:candidates.length,resolved,rejected,results,policy:"Published products with fewer than five approved images are refreshed with up to five high-confidence exact Google Images matches across product, packaging/contents and colour-variant searches. Customer storefront accepts approved gallery records only."});
