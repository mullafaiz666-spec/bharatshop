...
export async function POST(req:Request){
 try{
  ...
  const imgs=await db.select().from(productImages);const galleryReady=new Set(imgs.filter(x=>["WEB_IMAGE_EXACT_MATCH","WEB_SEARCH_MATCHED","VERIFIED"].includes(String(x.verificationStatus))&&!BAD.test(x.imageUrl)).map(x=>x.productId));
  const candidates=all.filter(p=>!galleryReady.has(p.id)).slice(0,limit);let resolved=0,rejected=0;const results:Array<Record<string,unknown>>=[];
  for(const p of candidates){
   ...
   await db.insert(productImages).values(selected.map((item:any,index:number)=>({productId:p.id,imageUrl:item.original!,sourceUrl:item.link!,sortOrder:index,altText:item.title||`${p.title} view ${index+1}`,verificationStatus:"WEB_IMAGE_EXACT_MATCH"})));
  ...
  return NextResponse.json({status:"COMPLETED",processed:candidates.length,resolved,rejected,results,policy:"Published products are given up to five high-confidence Google Images matches; customer storefront accepts only approved gallery records."});
