...
async function runMaintenance(limit:number){
 const[all,imgs,details]=await Promise.all([
  db.select().from(products).where(or(eq(products.status,"Published"),eq(products.status,"STAGED"),eq(products.status,"CEO_PENDING"))).orderBy(desc(products.id)),
