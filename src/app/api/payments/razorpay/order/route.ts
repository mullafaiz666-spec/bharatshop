...
export async function POST(req:Request){
 try{
  const body=await req.json(),refs=Array.from(new Set((Array.isArray(body.orderRefs)?body.orderRefs:[body.orderRef]).map((x:unknown)=>String(x||"").trim()).filter(Boolean))).slice(0,20);if(!refs.length)return NextResponse.json({error:"orderRef or orderRefs is required"},{status:400});
  const rows=[] as any[];for(const ref of refs){const[row]=await db.select().from(storefrontOrders).where(eq(storefrontOrders.orderRef,ref)).limit(1);if(!row)return NextResponse.json({error:`Order ${ref} not found`},{status:404});rows.push(row);}
  ...
  for(const o of rows){const notes=appendPaymentMeta(o.notes,{razorpay_order_id:created.orderId,gateway_amount_inr:total.toFixed(2)});await db.update(storefrontOrders).set({notes,paymentStatus:String(o.paymentMode).startsWith("PARTIAL_COD_")?"TOKEN_PENDING":"PAYMENT_PENDING"}).where(eq(storefrontOrders.id,o.id));}
