...
export async function GET(req:Request){
 ...
 const all=await db.select().from(products).orderBy(desc(products.aiScore));const imageRows=await db.select().from(productImages);const detailRows=await db.select().from(productDetails);const detailMap=new Map(detailRows.map(x=>[x.productId,x]));const galleryMap=new Map<number,{url:string;label:string}[]>();
 for(const row of imageRows){const url=cleanUrl(row.imageUrl);if(!url||!APPROVED.has(String(row.verificationStatus)))continue;const current=galleryMap.get(row.productId)||[];if(!current.some(x=>x.url===url))current.push({url,label:String(row.altText||"").trim()});galleryMap.set(row.productId,current.sort((a,b)=>a.url===b.url?0:0).slice(0,8));}
 ...
 const customerProducts=paginated.map(p=>{const gallery=galleryMap.get(p.id)||[];const primary=cleanUrl(p.imageUrl);const imageUrls=Array.from(new Set([...(gallery.map(x=>x.url)),primary].filter(Boolean))).slice(0,8);const imageLabels=imageUrls.map(u=>gallery.find(x=>x.url===u)?.label||"");const d=detailMap.get(p.id);return{id:p.id,sku:p.sku,title:p.title,category:p.category,brand:p.brand,imageUrl:imageUrls[0]||"",imageUrls,imageLabels,sellingPriceInr:p.sellingPriceInr,mrpInr:p.mrpInr,stockCount:Number(p.stockCount)||0,aiMarketingCopy:p.aiMarketingCopy,details:d?{description:d.description,specificationsJson:d.specificationsJson,variantsJson:d.variantsJson,includedItems:d.includedItems,dimensions:d.dimensions,weight:d.weight,material:d.material,colorOptions:d.colorOptions,warranty:d.warranty,countryOfOrigin:d.countryOfOrigin,careInstructions:d.careInstructions}:null};});
