...
export async function GET(req:Request){
 ...
 const all=await db.select().from(products).orderBy(desc(products.aiScore));const imageRows=await db.select().from(productImages);const galleryMap=new Map<number,string[]>();
 for(const row of imageRows){const url=cleanUrl(row.imageUrl);if(!url||!APPROVED.has(String(row.verificationStatus)))continue;const current=galleryMap.get(row.productId)||[];if(!current.includes(url))current.push(url);galleryMap.set(row.productId,current.slice(0,5));}
 ...
 const customerProducts=paginated.map(p=>{const gallery=galleryMap.get(p.id)||[];const primary=cleanUrl(p.imageUrl);const imageUrls=Array.from(new Set([...(gallery.length?gallery:[]),primary].filter(Boolean))).slice(0,5);return{id:p.id,sku:p.sku,title:p.title,category:p.category,brand:p.brand,imageUrl:imageUrls[0]||"",imageUrls,sellingPriceInr:p.sellingPriceInr,mrpInr:p.mrpInr,stockCount:Number(p.stockCount)||0,aiMarketingCopy:p.aiMarketingCopy};});
