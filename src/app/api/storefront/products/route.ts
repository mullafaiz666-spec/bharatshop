...
export async function GET(req:Request){
 ...
 const requestedId=searchParams.get("id");const all=await db.select().from(products).orderBy(desc(products.aiScore));const imageRows=await db.select().from(productImages);const detailRows=await db.select().from(productDetails);const detailMap=new Map(detailRows.map(x=>[x.productId,x]));const galleryMap=new Map<number,{url:string;label:string;order:number}[]>();
 for(const row of imageRows){const url=cleanUrl(row.imageUrl);if(!url||!isVerifiedMedia(row))continue;const current=galleryMap.get(row.productId)||[];if(!current.some(x=>x.url===url))current.push({url,label:String(row.altText||"").trim(),order:Number(row.sortOrder)||0});galleryMap.set(row.productId,current.sort((a,b)=>a.order-b.order).slice(0,8));}
