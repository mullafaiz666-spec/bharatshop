...
const APPROVED=new Set(["AI_VISION_VERIFIED","LOCAL_EVIDENCE_VERIFIED"]);
const APPROVED_PROVIDERS=new Set(["local-ai","local-evidence"]);
const MIN_IMAGES=4;
...
export async function GET(req:Request){
 ...
 for(const row of imageRows){const url=cleanUrl(row.imageUrl);if(!url||!APPROVED.has(String(row.verificationStatus))||Number(row.verificationConfidence)<MIN_CONFIDENCE||!APPROVED_PROVIDERS.has(String(row.verificationProvider))||!row.verifiedAt)continue;const current=galleryMap.get(row.productId)||[];if(!current.some(x=>x.url===url))current.push({url,label:String(row.altText||"").trim(),order:Number(row.sortOrder)||0});galleryMap.set(row.productId,current.sort((a,b)=>a.order-b.order).slice(0,8));}
