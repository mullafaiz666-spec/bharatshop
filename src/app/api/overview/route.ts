...
export async function GET(req: Request) {
  ...
  // Paginated products with optional filter/sort
  ...
  // Filter
  let filteredForPage = allProductsForKpis as typeof allProductsForKpis & {
    sku?: string; title?: string; imageUrl?: string; supplierName?: string; supplierCity?: string;
    shippingCostInr?: string; gstPct?: string; mrpInr?: string; customMarginPct?: string; viralVelocityScore?: number;
    stockCount?: number; moq?: number; status?: string; aiMarketingCopy?: string; aiTargetAudience?: string; hsnCode?: string;
    autoRepriceEnabled?: boolean; returnsCount?: number; createdAt?: Date; updatedAt?: Date;
  }[];

  // For paginated full product list
  const allFullProducts = await db.select().from(products).orderBy(desc(products.aiScore));
  let pageProducts = allFullProducts;

  if (category && category !== "ALL") {
    pageProducts = pageProducts.filter(p => p.category === category);
  }
  if (search) {
  ...
  const catalogAvgScore = allProductsForKpis.length > 0
    ? Math.round(allProductsForKpis.reduce((a, p) => a + p.aiScore, 0) / allProductsForKpis.length)
    : 0;
  const total24hSales = allProductsForKpis.reduce((a, p) => a + (p.salesCount24h || 0), 0);

  // Category distribution
  const categoryDist: Record<string, number> = {};
  allProductsForKpis.forEach(p => {
  ...
  const sparkline14Days = [
    { day: "Day 1", revenue: 12800, profit: 4900, orders: 14 },
    { day: "Day 2", revenue: 18400, profit: 7100, orders: 21 },
    { day: "Day 3", revenue: 16200, profit: 6300, orders: 19 },
    { day: "Day 4", revenue: 24600, profit: 9400, orders: 28 },
    { day: "Day 5", revenue: 28900, profit: 11200, orders: 34 },
    { day: "Day 6", revenue: 26100, profit: 10100, orders: 31 },
    { day: "Day 7", revenue: 34200, profit: 13300, orders: 41 },
    { day: "Day 8", revenue: 39800, profit: 15400, orders: 47 },
    { day: "Day 9", revenue: 36500, profit: 14100, orders: 44 },
    { day: "Day 10", revenue: 44900, profit: 17400, orders: 53 },
    { day: "Day 11", revenue: 51200, profit: 19800, orders: 62 },
    { day: "Day 12", revenue: 47600, profit: 18400, orders: 58 },
    { day: "Day 13", revenue: 58400, profit: 22600, orders: 70 },
    { day: "Day 14", revenue: 67200, profit: 26100, orders: 82 },
  ];
  ...
  return NextResponse.json({
    ...
    kpis: {
      ...
      totalSupplierCostInr: Number(totalCost.toFixed(2)),
      avgMarginPct,
      autoFulfillRatePct: autoFulfillRate,
      activeProductsCount: allProductsForKpis.length,
      pendingOrdersCount: allOrders.filter(o => ["Incoming", "AI Checking"].includes(o.fulfillmentStatus)).length,
      storesConnected: allStores.length,
      cartItemsCount: allCart.length,
      cartTotalCostInr: Number(cartTotalCost.toFixed(2)),
      cartProjectedRevenueInr: Number(cartProjectedRevenue.toFixed(2)),
      cartProjectedProfitInr: Number(cartProjectedProfit.toFixed(2)),
      catalogAvgAiScore: catalogAvgScore,
      catalogTotalProjectedProfitInr: Math.round(catalogTotalProfit),
      total24hSalesAcrossCatalog: total24hSales,
      totalCampaigns: campaigns.length,
      totalImpressions,
      totalClicks,
      totalConversions,
      totalCampaignRevenueInr: Math.round(totalCampaignRevenue),
...
export async function POST(req: Request) {
  try {
    ...
    await db.insert(aiActivityLogs).values({
      ...
      message: aiAutoPilotEnabled
        ? "AI Auto-Pilot ENGAGED — Scout, Delhivery auto-book, Daily Refresh sabhi systems active."
        : "AI Auto-Pilot Manual Review mode. Auto-fulfillment paused.",
