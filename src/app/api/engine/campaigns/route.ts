...
export async function GET(req: Request) {
  ...
  return NextResponse.json({ campaigns: filtered, total: filtered.length, configured: filtered.filter(c => channelConnected(c.platform)).map(c => c.platform) });
}
...
export async function POST(req: Request) {
  try {
    ...
    const status = "DRAFT";
    const [created] = await db.insert(marketingCampaigns).values({ userId, productId: targetProduct.id, productTitle: targetProduct.title, platform: selectedPlatform, campaignType: campaignType || campaign.campaignType, headline: customHeadline || campaign.headline, bodyText: customBody || campaign.bodyText, ctaText: campaign.ctaText, targetAudience: campaign.targetAudience, budgetInr: (budgetInr || campaign.budgetInr).toString(), estimatedReachK: campaign.estimatedReachK, estimatedRoas: campaign.estimatedRoas.toFixed(2), status, impressions: 0, clicks: 0, conversions: 0, revenueGeneratedInr: "0.00", scheduledAt: null }).returning();
    await db.insert(aiActivityLogs).values({ userId, agentName: "Advertising-Agent", actionType: "CAMPAIGN_DRAFTED", message: `${"Campaign prepared as draft; approval and provider publication required"}: ${created.headline.slice(0, 60)} on ${created.platform}.`, profitImpactInr: "0", status: connected ? "SUCCESS" : "BLOCKED", metadataJson: { connected, platform: selectedPlatform } });
    return NextResponse.json({ campaign: created, channelConfigured: connected, channelConnected: false, live: false, message: "Campaign saved as DRAFT. Credentials do not authorize spending or prove publication." }, { status: 201 });
...
export async function PATCH(req: Request) {
  try {
    ...
    if (!["DRAFT", "CANCELLED"].includes(status)) return NextResponse.json({ error: "Only local draft/cancel edits are allowed. Live, scheduled and paused status require a verified provider operation." }, { status: 409 });
