...
async function operatorAuthorized(req: Request) {
...
export async function POST(req:Request){
 try{
  ...
  if(!await operatorAuthorized(req))return NextResponse.json({error:"Human operator authorization required"}, {status:403, headers:{"Cache-Control":"no-store"}});
  const id=Number(body.id); if(!Number.isInteger(id))return NextResponse.json({error:"Valid approval id required"},{status:400});
  if(action==="approve"&&!process.env.BHARATSHOP_AUTOMATION_TOKEN)return NextResponse.json({error:"Automation execution is not configured"},{status:503});
  ...
  const exec=await fetch(`${origin}/api/agent-execute`,{method:"POST",headers:{"Content-Type":"application/json",authorization:`Bearer ${process.env.BHARATSHOP_AUTOMATION_TOKEN}`},body:JSON.stringify({actionType:approval.action_type,payload:approval.payload,agentName:approval.requested_by,approvalId:id}),cache:"no-store"});
  ...
  await audit(String(approval.requested_by||"AI CEO"),"APPROVAL_EXECUTION_RESULT",succeeded?"EXECUTED_UNVERIFIED":"FAILED",succeeded?`Approved ${approval.action_type} executed.`:`Approved ${approval.action_type} failed; no success is claimed.`,{approval,execution},id);
