...
export async function GET() {
  try {
    ...
    const trackingRecorded = trackingOrders.some(o => Boolean(o.tracking && String(o.tracking).trim()));
    const realOrder = trackingOrders.length > 0;
    const learningObserved = hasEvent(["LEARNING_UPDATED", "ORDER_LEARNING_RECORDED", "ORDER_STATUS_CHANGED", "SUPPLIER_PURCHASE_CONFIRMED"]);
    ...
    const checks = {
      ...
      advertise: { status: marketingPrepared ? "PREPARED" : "NOT_OBSERVED", message: marketingPrepared ? "Campaign preparation exists. LIVE still requires a connected real advertising account." : "No campaign preparation observed." },
