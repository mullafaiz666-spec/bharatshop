  ...
  try {
    ...
    for (const { product, details } of selected) {
      try {
        ...
        if (!sources.length) {
          results.push({ productId: product.id, status: "NO_EVIDENCE" });
          continue;
        }
        ...
        const evidenceBacked = Boolean(
          ...
          String(out.includedItems || "").trim() ||
          String(out.dimensions || "").trim() ||
          String(out.weight || "").trim() ||
          String(out.material || "").trim() ||
          String(out.warranty || "").trim() ||
          String(out.countryOfOrigin || "").trim() ||
          String(out.careInstructions || "").trim() ||
          variants.length
        );
        if (!evidenceBacked) {
          results.push({ productId: product.id, status: "NO_EXTRACTABLE_FACTS" });
          continue;
        }

        const oldSpecs = details.specificationsJson && typeof details.specificationsJson === "object" && !Array.isArray(details.specificationsJson)
          ? details.specificationsJson as Record<string, unknown>
          : {};
        const mergedSpecs = {
          ...oldSpecs,
          ...specifications,
          verifiedEnrichmentSources: sources.map((s: any) => s.link),
          enrichmentProvider: "SearXNG+local-Gemma",
          enrichedAt: new Date().toISOString(),
        };

        await db.update(productDetails).set({
          description: String(out.description || details.description || ""),
          specificationsJson: mergedSpecs,
          variantsJson: variants.length ? variants : details.variantsJson,
          includedItems: String(out.includedItems || details.includedItems || ""),
          dimensions: String(out.dimensions || details.dimensions || ""),
          weight: String(out.weight || details.weight || ""),
          material: String(out.material || details.material || ""),
    ...
    return NextResponse.json({
      ...
      enriched: results.filter(x => x.status === "ENRICHED").length,
      errors,
      results,
...
export async function GET() {
  ...
  return NextResponse.json({
    ...
    status: process.env.SEARXNG_URL && ai ? "ready" : "blocked_missing_provider",
    provider: "SearXNG+local-Gemma",
