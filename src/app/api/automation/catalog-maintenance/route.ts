...
function qualifiesImage(x: typeof productImages.$inferSelect) {
  return String(x.verificationStatus) === "AI_VISION_VERIFIED" && !BAD.test(x.imageUrl) && /^https:\/\//i.test(x.imageUrl) && Number(x.verificationConfidence) >= MIN_CONFIDENCE && String(x.verificationProvider) === "anthropic" && String(x.verificationModel) === ANTHROPIC_MODEL && !!x.verifiedAt;
}
...
export async function GET() {
  return NextResponse.json({ agent: "Product-Research-and-Catalogue-Agent", automation: "catalog-maintenance", status: process.env.SEARXNG_URL && process.env.ANTHROPIC_API_KEY ? "ready" : "blocked_missing_keys", provider: "postgres-staging->searxng->claude-vision", publicationPolicy: "STAGED until central Media Resolver proves >=4 AI_VISION_VERIFIED images with persisted Claude evidence" });
}
...
export async function POST(req: Request) {
  try {
    ...
    return NextResponse.json({ status: "COMPLETED", provider: "postgres-staging->searxng->claude-vision", mode: "maintenance", staged: 0, processed: results.length, resolved: results.filter((r: any) => r.status === "COMPLETE_MEDIA_RESOLVED").length, blocked: results.filter((r: any) => r.publicationGate === "BLOCK").length, gateBefore, gateAfter, results, policy: "This route cannot publish directly. A product becomes Published only inside the central Media Resolver after 4-8 reachable AI_VISION_VERIFIED images with persisted Claude evidence." });
  } catch (e) {
