...
async function enforcePublicationGate() {
  ...
  for (const x of imgs) {
    if (String(x.verificationStatus) === "AI_VISION_VERIFIED" && !BAD.test(x.imageUrl) && /^https?:\/\//i.test(x.imageUrl)) {
  ...
  for (const p of blocked) {
    await db.update(products).set({ status: "STAGED", updatedAt: new Date() }).where(eq(products.id, p.id));
  }
  return {
...
export async function GET() {
  return NextResponse.json({
    ...
    provider: "postgres-staging->searxng->claude-vision",
    publicationPolicy: "STAGED until central Media Resolver proves >=4 AI_VISION_VERIFIED images",
  });
}
...
export async function POST(req: Request) {
  try {
    ...
    const all = await db
      ...
      .from(products)
      .where(or(eq(products.status, "STAGED"), eq(products.status, "Published")))
    ...
    return NextResponse.json({
      ...
      gateAfter,
      results,
      policy: "This route cannot publish directly. A product becomes Published only inside the central Media Resolver after 4-8 reachable AI_VISION_VERIFIED images.",
    });
  } catch (e) {
