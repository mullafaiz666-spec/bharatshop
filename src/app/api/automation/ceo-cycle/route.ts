...
async function runCycle(req: Request) {
  ...
  for (const product of pending) {
    ...
    const evidence = {
      ...
      marginPct: Number(margin.toFixed(2)), minMarginPct: MIN_MARGIN_PCT,
    };
  ...
  const realOrders = await db.select({ id: storefrontOrders.id, orderRef: storefrontOrders.orderRef, fulfillmentStatus: storefrontOrders.fulfillmentStatus })
    .from(storefrontOrders).orderBy(desc(storefrontOrders.orderedAt)).limit(10);
  results.orders = realOrders.length
    ? { realOrderCount: realOrders.length, humanInteractionGate: true, orders: realOrders }
    : { realOrderCount: 0, humanInteractionGate: false };
  if (realOrders.length) await audit("CEO-Agent", "REAL_ORDER_HUMAN_GATE", "READY", "A real storefront order exists; CEO order review is eligible. No fulfillment action is taken by the timer alone.", results.orders);
