    ...
    designs: {
      ...
      items: {
        ...
        properties: {
          ...
          priceTargetInr: { type: "number" },
          aiScore: { type: "integer" },
          viralVelocityScore: { type: "integer" },
...
async function createQikinkProduct(design: any, image: string) {
  const url = process.env.QIKINK_PRODUCT_CREATE_URL;
  const token = process.env.QIKINK_API_KEY;
  if (!url || !token) return { status: "QIKINK_PENDING", reason: "QIKINK_PRODUCT_CREATE_URL/QIKINK_API_KEY not configured" };
  ...
  const response = await fetch(url, {
    ...
    body: JSON.stringify({
      ...
      design: design.designBrief,
      printMethod: design.printMethod,
      colorPalette: design.colorPalette,
      image,
      source: "bharatshop-ai-fashion-designer"
    }),
    signal: AbortSignal.timeout(30000),
  });
  const text = await response.text();
  if (!response.ok) throw new Error(`Qikink ${response.status}: ${text.slice(0, 800)}`);
  return { status: "QIKINK_CREATED", response: JSON.parse(text) };
}
...
async function run(count: number) {
  ...
  for (const design of plan.designs.slice(0, safeCount)) {
    try {
      ...
      const cost = Number(process.env.QIKINK_DEFAULT_PRODUCT_COST_INR || 265);
      const shipping = Number(process.env.QIKINK_DEFAULT_SHIPPING_INR || 54);
      const target = Math.max(Number(design.priceTargetInr || 999), Math.ceil((cost + shipping) / 0.55));
      const profit = Math.max(0, target - cost - shipping);
      ...
      let qikink: any = { status: "NOT_ATTEMPTED" };
      if (status === "AI_DRAFT" && process.env.QIKINK_PRODUCT_CREATE_URL && process.env.QIKINK_AUTO_CREATE_PRODUCTS === "true") {
        qikink = await createQikinkProduct(design, image);
      }

      const inserted = await pool.query(`INSERT INTO products (user_id,sku,title,category,image_url,brand,supplier_name,supplier_city,supplier_cost_inr,shipping_cost_inr,gst_pct,selling_price_inr,mrp_inr,custom_margin_pct,net_profit_inr,ai_score,viral_velocity_score,status,ai_marketing_copy,ai_target_audience,stock_count,moq) VALUES (1,$1,$2,$3,$4,'BharatShop AI','Qikink','India',$5,$6,5,$7,$8,$9,$10,$11,$12,$13,$14,$15,0,1) RETURNING id`, [sku, design.title, design.category, image, cost, shipping, target, Math.ceil(target * 1.2), margin, profit, Math.round(design.aiScore || 70), Math.round(design.viralVelocityScore || 70), status, `${design.trendReason}\nUGC hook: ${design.ugcHook}`, design.targetAudience]);
      ...
      await logGoogleMedia(productId, "FASHION_DESIGN", { title: design.title, aiScore: design.aiScore, qikink: qikink.status, status, marketSummary: plan.marketSummary });
      results.push({ productId, sku, title: design.title, status, margin: Math.round(margin), qikink: qikink.status });
    } catch (error) {
      results.push({ title: design.title, status: "FAILED", error: error instanceof Error ? error.message : "unknown error" });
    }
  }

  return { success: true, provider: "google-gemini", requested: safeCount, generated: results.filter((x) => x.status !== "FAILED").length, marketSummary: plan.marketSummary, results };
}

export async function GET(req: Request) {
  if (!authorized(req)) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  return NextResponse.json({ status: "READY", agent: "AI Fashion Designer", dailyTarget: "10-25", imageProvider: "Google Gemini / Nano Banana 2", ugcVideoProvider: "Google Veo", supplier: "Qikink", autoPublish: process.env.QIKINK_AUTO_CREATE_PRODUCTS === "true" });
}
