...
const ALLOWED:Record<string,string>={
  ...
  "SOURCE_VERIFY":"/api/agents/source-verify",
...
async function audit(agent:string,event:string,status:string,summary:string,evidence:any,approvalId?:number){
 ...
 await pool.query(`INSERT INTO agent_audit_records(agent_name,event_type,status,summary,evidence,approval_id) VALUES($1,$2,$3,$4,$5,$6)`,[agent,event,status,summary,JSON.stringify(evidence||{}),approvalId||null]);
}
export async function POST(req:Request){
 try{
  ...
  await audit(agent,"ACTION_EXECUTION",ok?"SUCCESS":"FAILED",ok?`Approved ${action} executed successfully.`:`Approved ${action} returned a failure or blocked result.`,{action,payload,result:data,httpStatus:response.status},body.approvalId?Number(body.approvalId):undefined);
