...
test("CEO chat is model-led and never impersonates Gemma with a canned fallback", () => {
  ...
  assert.match(route, /gemma-plan-act-observe/);
  assert.match(route, /Model selected a tool outside the agent permission set/);
...
test("CEO UI exposes the real Gemma runtime state", () => {
  ...
  assert.match(ui, /d\.modelStatus === "completed"/);
});
