...
function setup({ failTable, price = '100.00' } = {}) {
 ...
 const db = { async transaction(fn) {
   ...
   const tx = {
    async execute(query) {
      ...
      if (/UPDATE products/i.test(statement)) {
        const qty = Number(query.values?.[0] || 0);
        if (/stock_count\s*=\s*stock_count\s*-/i.test(statement)) {
          if (draftStock < qty) return { rows: [] };
          draftStock -= qty;
    ...
    select() { return { from(table) { return { where(condition) { return { async limit() { if (table.name === 'products') return [{...product, stockCount: draftStock}]; if (table.name === 'productDetails') return []; return draft[table.name].filter(row => row[condition.column] === condition.value); } }; } }; } }; },
 ...
 const { POST } = evaluate(route, {
  '@/db': { db }, '@/db/schema': tables, '@/lib/admin-auth': { getAdminUser: async () => null }, '@/lib/payments/token-plan': tokenPlan, '@/lib/orders/inventory-reservation': inventory,
 ...
 return { state, counts: () => ({transactions,locks,stockCount}), request: (body = payload, key = 'checkout_test_key_123') => POST(new Request('http://localhost/api/storefront/orders', { method: 'POST', headers: key ? {'Idempotency-Key':key} : {}, body: typeof body === 'string' ? body : JSON.stringify(body) })) };
}
test('order, backend record and audit commit together with server-derived totals', async () => {
 ...
 assert.deepEqual(app.counts(),{transactions:1,locks:1,stockCount:8});
});
for (const failTable of ['orders','aiActivityLogs']) test(`failure writing ${failTable} rolls back all records and hides database details`, async () => {
 ...
 assert.equal(app.counts().stockCount,10);
});
