...
test('UI pairs, previews, submits model and verification settings, and supports V1 fallback', async () => {
  class Element {
    constructor() { this.value = ''; this.checked = false; this.options = []; this.children = []; this.classList = { toggle() {} }; }
  ...
  elements.mode.value = 'auto'; elements['task-filter'].value = 'all'; elements['verify-after'].checked = true;
  let version = '2.0.0'; const requests = []; let jobs = [];
  const context = vm.createContext({
    document: { getElementById: id => { assert.ok(elements[id], `missing HTML element ${id}`); return elements[id]; }, createElement: () => new Element(), querySelectorAll: () => [], addEventListener() {} },
