...
function openSettings() { $('settings').classList.add('open'); $('scrim').hidden = false; $('key').focus(); }
function closeSettings() { $('settings').classList.remove('open'); $('scrim').hidden = true; }
$('settings-toggle').onclick = openSettings;
$('settings-close').onclick = closeSettings;
$('scrim').onclick = () => { closeSettings(); $('sidebar').classList.remove('open'); };
$('menu-toggle').onclick = () => { $('sidebar').classList.toggle('open'); $('scrim').hidden = !$('sidebar').classList.contains('open'); };
$('connection').onclick = openSettings;
openSettings();
...
function showHealth(health) {
  ...
  if (!selection && (health.models || []).includes('deepseek-coder-v2:16b')) $('brain').value = 'deepseek-coder-v2:16b';
...
function render() {
  ...
  if (!jobs.length) { const empty = document.createElement('div'); empty.className = 'empty'; empty.textContent = 'Connected. Ready for your first message.'; container.append(empty); return; }
  for (const job of jobs) {
    const item = document.createElement('button'); item.type = 'button'; item.textContent = job.task; item.title = job.task;
    item.onclick = () => { document.getElementById('job-' + job.id)?.scrollIntoView({ behavior: 'smooth', block: 'start' }); $('sidebar').classList.remove('open'); $('scrim').hidden = true; };
    history.append(item);
  }
  for (const job of [...jobs].reverse()) {
    const article = document.createElement('article'); article.className = 'job'; article.id = 'job-' + job.id;
    const user = document.createElement('div'); user.className = 'user-message'; user.textContent = job.task;
    const assistant = document.createElement('div'); assistant.className = 'assistant-message';
    const avatar = document.createElement('div'); avatar.className = 'avatar'; avatar.textContent = 'J';
    const reply = document.createElement('div'); reply.className = 'reply';
    const header = document.createElement('div'); header.className = 'reply-head'; header.append(document.createTextNode('Jarvis'));
    const status = document.createElement('span'); status.className = 'status ' + job.status; status.textContent = job.status.replaceAll('_', ' ');
    ...
    const meta = document.createElement('div'); meta.className = 'meta'; meta.textContent = `${job.route} · ${job.model || 'configured model'} · ${seconds}s · ${new Date(job.createdAt).toLocaleString()}${job.exitCode !== null ? ' · exit ' + job.exitCode : ''}`;
    const answer = document.createElement('div'); answer.className = 'answer'; answer.textContent = (job.output || 'Working on your request…').replace(/^Starting (chat|build|verify|browser|agency|status|company) worker…\s*Route: [^\n]*\n?/i, '').trim() || 'Working on your request…';
    ...
    reply.append(header, answer, meta);
    ...
    if (job.status === 'running') { const stop = document.createElement('button'); stop.textContent = 'Stop task'; stop.onclick = () => stopTask(job.id); actions.append(stop); }
    reply.append(actions);
    const details = document.createElement('details'); const summary = document.createElement('summary'); summary.textContent = 'Execution details'; const output = document.createElement('pre'); output.textContent = job.output || 'Waiting for worker output…'; details.append(summary, stages, output); reply.append(details);
    assistant.append(avatar, reply); article.append(user, assistant);
...
async function submit() {
  ...
  try {
    ...
    $('jobs').lastElementChild?.scrollIntoView?.({ behavior: 'smooth', block: 'end' });
  } catch (error) { notice(error.message); }
